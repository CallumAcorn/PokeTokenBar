...
final class UsageStore {
    ...
    /// Five-hour (session) account window, the instrument for external-usage credit.
    /// It rolls — its delta nets new usage against usage ageing out — but a rolling window cannot
    /// rise without new consumption arriving, aging out only ever lowers it, so a rise still implies
    /// real usage (it merely understates it). The seven-day window that used to feed this ticks far
    /// too rarely to register light Chat/Design sessions at all. See `ExternalUsageCredit`.
    var externalCreditPercent: Double? { limits?.fiveHour?.utilization }
