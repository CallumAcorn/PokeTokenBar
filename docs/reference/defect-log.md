...
- **사람의 관례(커밋 메시지)로 판정하는 가드는 조용히 안 걸린다 — 무엇이 바뀌었는지로 판정한다.**
  에셋 게이트는 "UI 를 건드린 커밋 제목이 `feat:` 인가"로 새 화면을 찾았다. 결과: moves/TM, battles,
  gym badges 가 **전부 새 화면인데 세 릴리스 연속 통과**했다(기여자가 conventional commits 를 쓰지
  않는다는 이유 하나로). 정작 처음 걸린 건 업스트림에서 체리픽한 설정 드롭다운(#212) — 상류가 그 관례를
  쓰기 때문이다. 가드가 잡아야 할 것은 하나도 못 잡고, 제일 안 중요한 것에서 처음 울렸다.
  기여자의 커밋 관례는 우리가 정할 수 없다. 판정은 **UI 소스 diff**(`git diff -- Sources/.../UI/`)로 한다.
  일반화: 가드의 트리거를 고를 때 "이 신호를 누가 통제하는가"를 먼저 묻는다. 우리 통제 밖의 관례에
  의존하면, 안 걸리는 게 정상 동작처럼 보이고 아무도 눈치채지 못한다.
- **예외구는 '있다/없다'가 아니라 '만료되는가'로 설계한다.** 원래 이 게이트의 탈출구는 "커밋 타입을
  `feat` 이 아닌 걸로 바꿔 판단을 남겨라"였는데, **이미 푸시된 커밋에는 쓸 수 없다**(히스토리 재작성).
  `2.5.1-hardened.5` 가 정확히 그 상태로 막혔다. 환경변수 우회구를 두지 않은 판단 자체는 옳다 — 그런
  변수는 습관이 된다. 대신 `docs/reference/asset-waivers.md` 에 **버전을 명시한** 한 줄을 남기게 했다:
  기록이 리뷰에 보이고, 버전이 박혀 있어 다음 릴리스엔 자동으로 만료된다. 우회구의 위험은 존재가
  아니라 **영속성**이다.
