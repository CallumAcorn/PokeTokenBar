...
| [PokéAPI](https://pokeapi.co/): `pokeapi.co`, `graphql.pokeapi.co` | Pokémon species, evolution &amp; moves | runtime fetch; cached locally, never bundled |
...
- **The grant does not survive a Claude Code token refresh.** Claude Code rewrites its own Keychain item whenever it renews its OAuth token, and that rewrite clears the "Always Allow" you gave this app. Official limits then stop updating until you press refresh and allow it again. Heavy Claude Code use means more refreshes, so expect to renew it periodically. The app now says so on screen instead of just going quiet. Nothing here can prevent it: Claude Code owns that item.
